...
Describe **what must be true**, never **how to build it**. Never emit file paths,
symbols, code pointers, invented modules or endpoints, migrations, task breakdowns,
designs, or technical approaches. The implementer performs its own exploration.

Test every output line: would it remain true if the code were rewritten from
scratch? If not, remove it.
...
- **Read-only.** Do not create a worktree or branch, edit any file, commit, push,
  or run anything that mutates the repository or its remotes, including OpenSpec
  artifacts.
...
- Produce exactly one of two things: a refined brief, or at most three questions.
...
3. **Explore in OpenSpec explore mode.** Invoke the repository's
   `openspec-explore` skill (`.codex/skills/openspec-explore`). Start with
   `openspec list --specs`, `openspec list`, and `openspec show <item>` for the
   relevant main specs and active changes. Read code, tests, and history only to
   establish current product behavior, never to plan the implementation.
4. **Resolve what you can.** Most apparent ambiguity in a rough request is
   answered by precedent. Find the precedent.
...
reasonable answer. Decide those, and record the consequential ones as stated
...
and assumptions. The
implementer reads **this and not the original request**, so anything it needs
must be in it, including every answer already given in the thread.
...
- Asking for acceptance criteria the issue already implies and the code confirms.
- Asking a question whose answer you could have found in `git log`.
- Listing every uncertainty you noticed instead of only the blocking ones.
- Producing a brief that restates the issue without adding repository evidence.
